        ...
        <div id="basic_fee">
            <table class="listTable" style="white-space:nowrap;">
                <tr>
                    ...
                    <th class="freqMode">
                        <a href="#" class="sort" name="freqMode" hidefocus>配套基准价格(元/月)(不含税)</a>
                    </th>
                </tr>
                @if(isset($sup_fees))
                    @foreach($sup_fees as $sup_fee)
                        <tr>
                            {{--<td>--}}
                                {{--<a href="{{url('backend/rentStd/sup_fee_update/'.$sup_fee->seq.'/'.$filter['region'].'/'.$filter['fee_type'])}}">修改</a>--}}
                            {{--</td>--}}
                            <td>{{$sup_fee->tower_type}}</td>
                            <td>{{$sup_fee->product_type}}</td>
                            <td>{{$sup_fee->is_new_tower}}</td>
                            <td>{{$sup_fee->fee_sup}}</td>
                        </tr>
                    @endforeach
                @endif
