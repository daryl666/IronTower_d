...
if(typeof(KindEditor) != "undefined") {
	KindEditor.ready(function(K) {
		editor = K.create("#editor", {
			...
			uploadJson: cell.base + "/file!ajaxUpload.action",
			fileManagerJson: cell.base + "/file!ajaxBrowser.action",
...
function currencyFormat(price) {
	price = setScale(price, cell.priceScale, cell.priceRoundType);
	return cell.currencySign + price + cell.currencyUnit;
}
...
$().ready( function() {
	...
	if ($listForm.size() > 0) {
		...
		// 特定页面
		var $batchInvalidButton = $("#batchInvalidButton"); // 批量作废
		...
		$allCheck.click( function() {
			...
			if ($this.attr("checked")) {
				...
				$batchInvalidButton.attr("disabled", false);
				$listTableTr.addClass("checked");
			} else {
				$idsCheck.attr("checked", false);
				$deleteButton.attr("disabled", true);
				$batchInvalidButton.attr("disabled", true);
		...
		// 批量作废
		$batchInvalidButton.click( function() {
			var url = $(this).attr("url");
			var $idsCheckedCheck = $("#listTable input[name='ids']:checked");
			$.dialog({type: "warn", content: "您确定要作废吗?", ok: "确 定", cancel: "取 消", modal: true, okCallback: batchInvalid});
			function batchInvalid() {
				$.ajax({
					...
					success: function(data) {
						if (data.status == "success") {
							$idsCheckedCheck.parent().parent().find(".statusRow").html("已作废")
							$idsCheckedCheck.parent().parent().find(".invalidButton").remove();
						}
						$batchInvalidButton.attr("disabled", true);
						$allCheck.attr("checked", false);
						$idsCheckedCheck.attr("checked", false);
